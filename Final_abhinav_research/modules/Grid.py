...
class Grid:
    ...
    @property
    def weightMat(self):
        """returns a diagonal matrix with all weights"""
    ...
    def _rescaleGlPoints(self, start, end, weights, points):
        """
        rescaled the Gauss-Legendre grid `points` and `weights` from [-1,1]
        to [`start`, `end`]
        """
        tmp = (end - start)/2.
    ...
    def testQuadrature(self, eps=1e-9):
        """performs a self-test, checking integration of test functions against analytic results"""
    ...
    def print(self):
        """prints the grid and weights to the screen"""
        print(self)
